...
URL = r"C:\Users\dnguy\OneDrive\Desktop\eo_big_data.csv"
...
def download_data():
    ...
    # remove admin users
    # frame = frame.set_index("user_name")
    # frame = frame.drop(admin_users, axis=0)


    frame['date'] = pd.to_datetime(frame['date'])
    frame['accurate_to_date'] = pd.to_datetime(frame['date']).dt.to_period('D')
    # Return a subset of the columns
    return frame[["date", "user_name", "membership_used", "total_net_amount", "payment_type", "pickup_location", "drop_location"]]
